...
path1 = '/home/images/ipdetails.txt'
def main():
			...
			for i in range(0, len(nodesAvailable)):
				...
				if float(nodeDetailsData[0]) > 80:
					if float(nodeDetailsData[0]) < 95:
						#adminDecision = subprocess.check_output("java -jar /usr/jython/jython.jar /usr/jython/JythonMaster.py",shell=True)
						...
						if nodeNumber in nodeDetails:
							...
							if allVmsInNode[maxVmIndex] in vmDetails:
								...
								if nodeToMigrate == nodeNumber:
									...
									subprocess.call("ssh root@"+vmDetailsObtained[1]+" 'bash -s' < ./migrate.sh "+str(vmName)+" "+str(nodesAvailable[nodeToMigrate]), shell=True)
			...
			for i in range(0, len(vmAvailable)):
				...
				if float(vmDetailsData[0]) > 80 or float(vmDetailsData[0]) < 5:
					if float(vmDetailsData[0]) < 95 and float(vmDetailsData[0]) > 80:
						print 'Do you wanna scale up due to memory insufficient on '+str(vmNumber)+' ? >yes/no --> '
						userDecision = subprocess.check_output("java -jar /usr/jython/jython.jar /usr/jython/JythonMaster.py",shell=True).strip()
					elif float(vmDetailsData[0]) >= 0 and float(vmDetailsData[0]) < 5:
						print 'Do you wanna scale down due to memory not in use for '+str(vmNumber)+' ? >yes/no --> '
						userDecision = subprocess.check_output("java -jar /usr/jython/jython.jar /usr/jython/JythonMaster.py",shell=True).strip()
					else:
					...
					if userDecision == 'yes':
						...
						if vmNumber in vmDetails:
							...
							newValue = subprocess.check_output("java -jar /usr/jython/jython.jar /usr/jython/JythonMaster.py",shell=True).strip()
							print 'Change Memory for  '+str(vmNumber)+'from '+ vmDetailsObtained[3] +' to ? --> '
							...
							if(False):
							#if(float(nodeDataArray[0]) < (float(newValue)/float(nodeTotalDataArray[0]) * 100)):
								print 'Screwed Up'
				...
				if float(vmDetailsData[2]) > 80 or float(vmDetailsData[2]) < 5:
					if float(vmDetailsData[2]) < 95 and float(vmDetailsData[2]) > 80:
						print 'Do you wanna scale up due to cpu insufficient on '+str(vmNumber)+' ? >yes/no --> '
						userDecision = subprocess.check_output("java -jar /usr/jython/jython.jar /usr/jython/JythonMaster.py",shell=True).strip()
					elif float(vmDetailsData[2]) >= 0 and float(vmDetailsData[2]) < 5:
						print 'Do you wanna scale down due to cpu not in use for '+str(vmNumber)+' ? >yes/no --> '
						userDecision = subprocess.check_output("java -jar /usr/jython/jython.jar /usr/jython/JythonMaster.py",shell=True).strip()
					else:
						print 'Threshold level 2 crossed.. Lets Scale..'
						userDecision = 'yes'
					...
					if userDecision == 'yes':
						...
						if vmNumber in vmDetails:
							vmDetailsObtained = vmDetails[vmNumber]
							newValue = subprocess.check_output("java -jar /usr/jython/jython.jar /usr/jython/JythonMaster.py",shell=True).strip()
							print 'Change Cpus for  '+str(vmNumber)+'from '+ vmDetailsObtained[2] +'  to ? --> '

							nodeData = subprocess.check_output('python monitor.py --metric --all '+vmDetailsObtained[1] , shell=True)
							nodeDataArray = nodeData.split('\n')
							if(False):
							#if(float(nodeDataArray[2]) < 95):
								print 'Screwed Up'
				...
				if float(vmDetailsData[3]) > 80 or float(vmDetailsData[3]) < 5:
					if float(vmDetailsData[3]) < 95 and float(vmDetailsData[3]) > 80:
						print 'Do you wanna scale up due to disk insufficient on '+str(vmNumber)+' ? >yes/no --> '
						userDecision = subprocess.check_output("java -jar /usr/jython/jython.jar /usr/jython/JythonMaster.py",shell=True).strip()
					elif float(vmDetailsData[3]) >= 0 and float(vmDetailsData[3]) < 5:
						print 'Do you wanna scale down due to disk not in use for '+str(vmNumber)+' ? >yes/no --> '
						userDecision = subprocess.check_output("java -jar /usr/jython/jython.jar /usr/jython/JythonMaster.py",shell=True).strip()
					else:
						print 'Threshold level 2 crossed.. Lets Scale..'
						userDecision = 'yes'
					...
					if userDecision == 'yes':
						...
						if vmNumber in vmDetails:
							vmDetailsObtained = vmDetails[vmNumber]
							newValue = subprocess.check_output("java -jar /usr/jython/jython.jar /usr/jython/JythonMaster.py",shell=True).strip()
							print 'Change Disk for  '+str(vmNumber)+'from '+ vmDetailsObtained[4] +'  to ? --> '
							...
							nodeTotalDataArray = nodeTotalData.split('\n')
							if(False):
							#if(float(nodeDataArray[3]) < (float(newValue)/float(nodeTotalDataArray[2]) * 100)):
								print 'Screwed Up'
...
def executeMigration(from_node, to_node, vm_name):
	userDecision = subprocess.check_output("java -jar /usr/jython/jython.jar /usr/jython/JythonMaster.py",shell=True)
	print userDecision
...
def executeScalingUp(username, vm_name, size):
	...
	subprocess.call("sshpass -p 'cloud123' ssh -o StrictHostKeyChecking=no "+vm_name+" 'bash -s' < ./disk_scale1.sh ", shell=True)
	subprocess.call("sleep 60", shell=True)
	subprocess.call("sshpass -p 'cloud123' ssh -o StrictHostKeyChecking=no "+vm_name+" 'bash -s' < ./disk_scale2.sh ", shell=True)
...
def executeCreateVm(username, vm_name, memory, disk, os, cpu, io, ipadd):
	...
	if str(node).isdigit():
		...
		subprocess.call("sshpass -p 'admin' ssh -o StrictHostKeyChecking=no "+vm_name+" 'bash -s' < /home/images/install-gmond.sh ", shell=True)
...
def executeCreateExistingVm(username, vm_name, memory, disk, os, cpu, io, ipadd, image_name, job_type):
	...
	if str(node).isdigit():
		...
		subprocess.call("sshpass -p 'cloud123' ssh -o StrictHostKeyChecking=no "+vm_name+" 'bash -s' < /home/images/install-gmond.sh ", shell=True)
		addUserDetails(username, vm_name, ipadd, nodesAvailable[node], cpu, memory, disk, io, 'no-snapshot')
	else:
		print 'Sorry Vm Creation Not possible.. Try again later..'
...
def executeChangeMemoryVm(vm_name, size):
	...
	getVmDetails()
	nodeName = vmDetails[vm_name][1]
	originalMemory = vmDetails[vm_name][3]
	print "ORIGINAL MEMORY", originalMemory
	if originalMemory < size:
		subprocess.call("ssh root@"+nodeName+" 'bash -s' < ./changememory.sh "+vm_name+" "+size, shell=True)
	else:
...
def executeChangeCpuVm(vm_name, size):
	...
	if int(size) <= 8:
		...
		cores = int(size)/8
		threads = 8
	originalvCPUs = vmDetails[vm_name][2]
	if originalvCPUs < size:
		subprocess.call("ssh root@"+nodeName+" 'bash -s' < ./changecpuvm.sh "+vm_name+" "+size, shell=True)
	else:
...
def writeUserDetails():
	...
	for eachUser in userDetails:
		...
		for i in range(0,len(eachUserDetail)):
			...
			outdata.write('\n')
	outdata.close()
	outdata1 = open(path1, 'w')
	for eachUser in userDetails:
		eachUserDetail = userDetails[eachUser]
		for i in range(0,len(eachUserDetail)):
			printData = eachUserDetail[i][0] +','+ eachUserDetail[i][1]
			outdata1.write(printData)
			outdata1.write('\n')
	outdata1.close()
...
def addUserDetails(user_name, vm_name, IP_address, node_name, CPU, memory, disk, io, snapshot):
	...
	with open(path, 'a') as outdata:
		...
		outdata.write('\n')
	outdata.close()
	ipdata = str(vm_name) +','+ str(IP_address)
	with open(path1, 'a') as outdata1:
		outdata1.write(ipdata)
		outdata1.write('\n')
	outdata1.close()
